...
def train_dynamics(args, dynamics_learner,
                   optimizer, device, train_loader, epoch, use_cuda):
...
def val_dynamics(args, dynamics_learner, device, val_loader, epoch, best_val_loss, use_cuda):

    loss_record = []
    mse_record = []
    for batch_idx, data in enumerate(val_loader):
        data = data.to(device)
        loss, mse = val_dynamics_learner(dynamics_learner, data, args.prediction_steps, use_cuda, device)
        loss_record.append(loss.item())
        mse_record.append(mse.item())
...
def train_dynamics_learner(optimizer, dynamics_learner, data, steps, use_cuda, device):
    ...
    for t in range(steps - 1):
        ...
        output = output1 + output3*0.05 - output1 * 0.05 * 1
...
def val_dynamics_learner(dynamics_learner, data, steps, use_cuda, device):
...
def test(args, dynamics_learner, device, test_loader, use_cuda):
    # load model
...
def main():
    ...
    parser.add_argument('--prediction-steps', type=int, default=200,
    ...
    test(args, dynamics_learner, device, test_loader, use_cuda)
