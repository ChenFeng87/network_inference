...
def load_data(batch_size=128):
    ...
    with open(data_path, 'rb') as f:
        train_data, val_data, test_data, train_ab, val_ab, test_ab = pickle.load(f)
    ...
    train_ab_loader = DataLoader(train_ab, batch_size=batch_size, shuffle=False)
    val_ab_loader = DataLoader(val_ab, batch_size=batch_size, shuffle=False)
    test_ab_loader = DataLoader(test_ab, batch_size=batch_size, shuffle=False)

    return train_loader, val_loader, test_loader, train_ab_loader, val_ab_loader, test_ab_loader


def train_dynamics(args, dynamics_learner,
                   optimizer, device, train_loader, use_cuda, train_ab_loader):
    ...
    for step in range(1, args.sub_epochs + 1):
        loss_record = []
        mse_record = []
        for batch_idx, (data, data_ab) in enumerate(zip(train_loader, train_ab_loader)):
            data = data.to(device)
            data_ab = data_ab.to(device)
...
def val_dynamics(args, dynamics_learner, device, val_loader, best_val_loss, use_cuda, val_ab_loader):

    loss_record = []
    mse_record = []
    for batch_idx, (data, data_ab) in enumerate(zip(val_loader, val_ab_loader)):
        data = data.to(device)
        data_ab = data_ab.to(device)
        loss, mse = val_dynamics_learner(dynamics_learner, data, args.prediction_steps, use_cuda, device, data_ab)
        loss_record.append(loss.item())
        mse_record.append(mse.item())
...
def train_dynamics_learner(optimizer, dynamics_learner, data, steps, use_cuda, device, data_ab):
    optimizer.zero_grad()
    ...
    for t in range(steps - 1):
        ...
        data_ab1 = torch.as_tensor(data_ab, dtype=torch.float32).view(-1, 2).to(device)
        output2 = torch.cat([output1, data_ab1], dim=1)
        output3 = dynamics_learner(output2)
        output = output1 + output3*0.5 - output1 * 0.5 * 1
...
def val_dynamics_learner(dynamics_learner, data, steps, use_cuda, device, data_ab):
...
def test(args, dynamics_learner, device, test_loader, use_cuda, test_ab_loader):
    # load model
    dynamics_learner.load_state_dict(torch.load(args.dynamics_path))
    dynamics_learner.eval()
    loss_record = []
    mse_record = []
    for batch_idx, (data, data_ab) in enumerate(zip(test_loader, test_ab_loader)):
        data = data.to(device)
        data_ab = data_ab.to(device)
        loss, mse = val_dynamics_learner(dynamics_learner, data, args.prediction_steps, use_cuda, device, data_ab)
        loss_record.append(loss.item())
...
def main():
    ...
    parser.add_argument('--prediction-steps', type=int, default=15,
                        help='prediction steps in data (default: 15)')
    ...
    # Loading data
    print('\n----------   Loading data ----------')
    train_loader, val_loader, test_loader, train_ab_loader, val_ab_loader, test_ab_loader \
    ...
    test(args, dynamics_learner, device, test_loader, use_cuda, test_ab_loader)
